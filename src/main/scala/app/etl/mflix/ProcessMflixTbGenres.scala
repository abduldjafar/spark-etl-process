...
object ProcessMflixTbGenres {
  def main (args: Array [String]): Unit = {
    ...
    val spark =
      init.sessionSPark (hiveSupport = hiveSupport, useAwsEnv = useAwsEnv)
    ...
    AfterProcessing.writeToS3asParquet (
      mflix_genres_tables, init.mflix_movies_genres_s3_path, "overwrite",
      init.mflix_movie_genres_tb_location, "genre", true, hiveSupport
    )

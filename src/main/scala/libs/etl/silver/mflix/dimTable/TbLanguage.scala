...
object TbLanguage {
  def get(mflix_dataframe: DataFrame): DataFrame = {
    var result = mflix_dataframe.select("oid__id", "array_languages")

    result
      .withColumn(
        "array_languages",
        explode(
          from_json(col("array_languages"), ArrayType(StringType))
        )
      )
      .withColumnRenamed("oid__id", "movie_id")
      .withColumnRenamed("array_languages", "languages")
      .withColumn("languages", lower(col("languages")))
  }
